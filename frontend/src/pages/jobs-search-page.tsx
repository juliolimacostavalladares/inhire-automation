...
export function JobsSearchPage() {
  ...
  const { jobs, total, selectedJob, favorites, loading, error, detailError, fetchJobs, selectJob, clearSelection, toggleFavorite } = useJobs()
  ...
  }, [clearSelection, fetchJobs, request])
  ...
  return (
    <div className="min-h-svh bg-canvas">
      <CandidateTopbar />
      <div className={cn(selectedJob && 'xl:grid xl:grid-cols-[minmax(0,1fr)_minmax(24rem,29rem)]')}>

      <main className={cn('min-w-0', selectedJob && 'xl:h-[calc(100svh-4.25rem)] xl:overflow-y-auto')}>
        <div
          className={cn(
            'mx-auto w-full px-5 py-7 sm:px-8 lg:px-9 lg:py-10 2xl:px-12',
            selectedJob ? 'max-w-4xl' : 'max-w-5xl',
          )}
        >
          <section
      ...
      {selectedJob && (
        <JobDetailPanel
          ...
          favorite={favorites.has(selectedJob.id)}
